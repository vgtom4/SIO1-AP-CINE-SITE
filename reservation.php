...
<body>
    ...
    if(isset($_GET["btnvalider"])==true) {
        ...
        while ($uneligne!=null)
        {
            $requete2 = ("select (nbplaces - (select sum(nbplacesresa) from reservation where noproj=$uneligne[noproj])) from salle natural join projection natural join reservation where nosalle=(select nosalle from projection where noproj =$uneligne[noproj]) ");
            $req2 = $bdd->prepare($requete2);
            
            $req2->execute();

            $uneligne2 = $req->fetchColumn();
